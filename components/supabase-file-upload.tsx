...
import { uploadFileToNotebook } from "@/app/actions/upload"
...
export function SupabaseFileUpload({
  ...
  const uploadFileToSupabase = async (fileObj: FileWithPreview) => {
    try {
      ...
      // Upload the file to Supabase via server action
      const result = await uploadFileToNotebook(notebookId, fileObj.file)
      ...
      if (result.success) {
        setFiles((prev) =>
          prev.map((f) =>
            f.id === fileObj.id ? { ...f, progress: 100, status: "complete", fileUrl: result.fileUrl } : f,
          ),
        )
      } else {
        setFiles((prev) => prev.map((f) => (f.id === fileObj.id ? { ...f, progress: 0, status: "error" } : f)))
      }
    } catch (error) {
